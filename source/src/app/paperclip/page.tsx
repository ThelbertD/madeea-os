      ...
      <div className="flex items-end justify-between mb-5 px-1">
        <div>
          <div className="flex items-center gap-2.5">
            <div className="grid place-items-center w-9 h-9 rounded-xl" style={{ background: "rgba(253, 88, 18,0.14)", border: "1px solid rgba(253, 88, 18,0.4)", color: "#fd5812" }}><Hammer size={17} /></div>
            ...
            <motion.div key={build.issueId} layout initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
              ...
              <a href={build.liveUrl ?? "#"} target="_blank" rel="noopener noreferrer"
                ...
                {build.liveUrl && (
                  <div className="absolute inset-0 grid place-items-center opacity-0 group-hover:opacity-100 transition" style={{ background: "rgba(5,8,13,0.45)" }}>
                    <div className="inline-flex items-center gap-2 px-4 py-2.5 rounded-xl text-[13px] font-semibold" style={{ background: "#fd5812", color: "#1a1206" }}><Play size={14} /> Open live build</div>
                  </div>
                )}
              ...
              <div className="p-3.5 flex flex-col gap-2 flex-1">
                ...
                <div className="flex items-center gap-2 mt-1">
                  {build.liveUrl && (
                    <a href={build.liveUrl} target="_blank" rel="noopener noreferrer"
                      className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[12px] font-medium" style={{ background: "rgba(253, 88, 18,0.16)", color: "#fd5812" }}><Play size={12} /> View live</a>
                  )}
        ...
        <div className="flex items-center gap-1 p-1 rounded-xl" style={{ background: "rgba(255,255,255,0.03)", border: "1px solid var(--panel-border)" }}>
          {VIEWS.map((v) => (
            <button key={v.key} onClick={() => { setErrored(false); setView(v.key); }}
              ...
              style={view === v.key ? { background: "rgba(253, 88, 18,0.16)", color: "#fd5812" } : { color: "var(--fg-dim)" }}>
