...
const MISSIONS: Mission[] = [
  ...
  {
    id: "showcase", label: "Build a showcase page", icon: <Rocket size={13} />, color: "#fd5812", project: "ultracode-showcase",
    prompt: "Build a dazzling single-file HTML showcase page at index.html for 'Agent OS'. Use parallel subagents: hero section, animated feature grid, CSS animation library, closing CTA — then assemble into one self-contained index.html. Dark midnight-aubergine theme (#09141f), gold (#fd5812) + pink (#f472b6) accents, premium animated, fully responsive, zero external deps.",
  },
  {
...
function RunDetail({ run, launching, onReply, onStop }: { run: UltracodeRun; launching: string | null; onReply: (text: string) => void; onStop: () => void }) {
  ...
  return (
    <div className="flex flex-col h-full min-h-0">
      ...
      <div className="flex-1 min-h-0 overflow-hidden relative" style={{ background: "radial-gradient(ellipse at center, rgba(253, 88, 18,0.05), #0a070d)" }}>
...
function SwarmMap({ run }: { run: UltracodeRun }) {
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} className="w-full h-full" preserveAspectRatio="xMidYMid meet">
      ...
      <text x={cx} y={cy + 11} textAnchor="middle" fontSize="8" fill="#a3b3c2">orchestrator</text>
      {run.status === "running" && (
      ...
      {positions.map((p, i) => {
        ...
        return (
          <motion.g key={`n-${a.taskId}`}
            ...
            {/* label below */}
            <text x={p.x} y={p.y + r + 14} textAnchor="middle" fontSize="9.5" fill="#f4f4f5">{label}</text>
            <text x={p.x} y={p.y + r + 25} textAnchor="middle" fontSize="8" fill="#a3b3c2">
      ...
      {n === 0 && (
        <text x={cx} y={cy + 60} textAnchor="middle" fontSize="12" fill="#a3b3c2">
