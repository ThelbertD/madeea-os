...
const COLS: { key: Stage | "done"; label: string; accent: string; stages: Stage[] }[] = [
  { key: "queued", label: "Backlog", accent: "#a3b3c2", stages: ["queued"] },
  { key: "building", label: "Building", accent: "#fd5812", stages: ["building"] },
...
const TEAM = [
  ...
  { key: "builder", name: "Builder", icon: Hammer, accent: "#fd5812", does: "builds each card" },
              ...
              <AnimatePresence>
                {cards.filter((c) => col.stages.includes(c.stage)).map((c) => (
                  <motion.div key={c.id} layout layoutId={c.id}
                    ...
                    {c.stage === "building" && <div className="mt-2 text-[10px] inline-flex items-center gap-1.5" style={{ color: "#fd5812" }}><Hammer size={10} className="animate-pulse" /> building…</div>}
