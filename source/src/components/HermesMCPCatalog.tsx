            ...
            <>
              ...
              {(manifest.installUrl || manifest.bootstrap?.length) && (
                <div className="rounded-md border p-3 text-[11px] space-y-1.5"
                     style={{ borderColor: "var(--line-soft)", background: "rgba(244, 244, 245,0.03)" }}>
              ...
              <div className="flex items-center justify-end gap-2 pt-2">
                <button onClick={onClose}
                        className="text-[11px] uppercase tracking-widest px-3 py-1.5 rounded hover:bg-[rgba(244, 244, 245,0.05)]"
                        style={{ color: "var(--cream-mute)" }}>
                  Cancel
                </button>
...
function ManifestPill({ version }: { version: number }) {
  return (
    <span className="text-[9.5px] uppercase tracking-[0.2em] px-1.5 py-0.5 rounded shrink-0 mono"
          style={{ color: "var(--cream-mute)", border: "1px solid var(--line-soft)", background: "rgba(244, 244, 245,0.04)" }}>
      manifest v{version}
    </span>
  );
}
      ...
      <div className="panel w-full max-w-[720px] max-h-[90vh] flex flex-col overflow-hidden"
        ...
        <div className="flex-1 min-h-0 overflow-y-auto scroll p-4 space-y-4">
          ...
          <div className="flex items-center justify-end gap-2 pt-1">
            <button onClick={onClose} disabled={submitting}
                    className="text-[11px] uppercase tracking-widest px-3 py-1.5 rounded hover:bg-[rgba(244, 244, 245,0.05)] disabled:opacity-50"
                    style={{ color: "var(--cream-mute)" }}>
              Cancel
            </button>
