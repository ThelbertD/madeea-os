        ...
        <AnimatePresence initial={false}>
          {msgs.map((m) => m.who === "you" ? (
            <motion.div key={m.key} initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} className="flex justify-end">
              <div className="max-w-[78%] rounded-2xl rounded-tr-sm px-3.5 py-2.5 text-[14px] leading-relaxed" style={{ background: "rgba(253, 88, 18,0.16)", border: "1px solid rgba(253, 88, 18,0.35)", color: "var(--fg)" }}>{m.text}</div>
            </motion.div>
          ) : m.who === "system" ? (
