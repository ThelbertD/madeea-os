...
const STATUS_STYLE: Record<Status, { label: string; color: string; bg: string; border: string }> = {
  ...
  doing: { label: "doing", color: "#ff7a42", bg: "rgba(253, 88, 18,0.14)", border: "rgba(253, 88, 18,0.4)" },
...
const GOLD = "var(--gold, #fd5812)";
const COLORS = ["#fd5812", "#5ab896", "#2dd4bf", "#a78bfa", "#ff7a42", "#c4607e"];
    ...
    <div ref={panelRef} className="surface-card p-5 relative overflow-hidden">
      ...
      {todos.length > 0 && (
        <div className="mb-4">
          <div className="flex items-center justify-between mb-1">
            <span className="text-[12px] mono tracking-widest uppercase font-semibold" style={{ color: allDone ? "var(--emerald)" : "var(--cream-dim, #a3b3c2)" }}>
              {allDone ? "day crushed ✓" : `${doneCount}/${tasks.length} done`}
            </span>
            <span className="text-[12px] mono font-semibold" style={{ color: allDone ? "var(--emerald)" : "var(--cream-dim, #a3b3c2)" }}>{pct}%</span>
          </div>
            ...
            <div className="h-full rounded-full transition-all duration-500"
              style={{
                ...
                background: allDone
                  ...
                  : "linear-gradient(90deg,#fd5812,#ff7a42,#5ab896)",
      ...
      {histOpen && (
        <div className="absolute right-4 top-14 z-40 w-[270px] max-h-[280px] overflow-y-auto scroll rounded-xl border shadow-2xl"
          style={{ background: "var(--bg-elev, #1c3247)", borderColor: "var(--line-soft)" }}>
      ...
      <div className="space-y-2">
        ...
        {todos.map((t, i) => {
          ...
          <div key={t.id} draggable={dragArmed === t.id}
            ...
            className={`group flex items-center gap-3 rounded-xl transition-colors ${heading ? "px-3 pt-4 pb-1" : "px-3 py-2.5 border hover:border-[rgba(253, 88, 18,0.35)]"}`}
            style={{
            ...
            {!heading && (
              <button onClick={(e) => toggle(t, e)}
                ...
                style={{ borderColor: t.done ? "var(--emerald)" : "rgba(244, 244, 245,0.35)", background: t.done ? "rgba(90,184,150,0.22)" : "transparent" }}
            ...
            {editingId === t.id ? (
              <input autoFocus value={editText} onChange={(e) => setEditText(e.target.value)}
                ...
                style={{ borderColor: "var(--gold, #fd5812)", color: "var(--cream)" }} />
            ) : heading ? (
              <span className="flex-1 leading-snug cursor-text select-text font-semibold"
                ...
                style={{ fontFamily: "'Bricolage Grotesque', var(--font-sans, sans-serif)", fontSize: 18, color: "var(--gold-soft, #ff7a42)", letterSpacing: "0.01em", userSelect: "text", WebkitUserSelect: "text" }}>
      ...
      <div className="flex items-center gap-2 mt-3 pt-3 border-t" style={{ borderColor: "var(--line-soft)" }}>
        ...
        <button onClick={add} disabled={!text.trim()}
          ...
          style={{ background: `linear-gradient(135deg, #fd5812, #ff7a42)`, color: "#2a1c12", opacity: text.trim() ? 1 : 0.5 }}>
          <Plus size={13} /> Add
        </button>
      </div>
    </div>
