...
const GOLD = "#fd5812", PINK = "#f472b6", BLUE = "#60a5fa", EMERALD = "#5ab896", PLUM = "#c4607e", MUTE = "#a3b3c2";
...
export default function RufloView() {
  ...
  return (
    <div className="space-y-4">
      ...
      {/* The graph */}
      <div className="panel p-0 overflow-hidden" style={{ minHeight: 540, background: "radial-gradient(ellipse at center, rgba(253, 88, 18,0.05), #0a070d)" }}>
        {agents.length === 0 ? (
      ...
      {pos.map((p, i) => {
        ...
        return (
          <motion.g key={`n${a.id}`} initial={{ scale: 0, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}
            ...
            <text x={p.x} y={p.y + r + 13} textAnchor="middle" fontSize="9.5" fill="#f4f4f5">{lab}</text>
