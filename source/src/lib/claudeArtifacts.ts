...
<style>
  body{margin:0;background:#09141f;color:#f4f4f5;font-family:'Manrope',system-ui,sans-serif;padding:48px 24px;}
  .wrap{max-width:880px;margin:0 auto;}
  h1{font-family:'Bricolage Grotesque',sans-serif;font-weight:500;font-size:2.2rem;margin:0 0 6px;}
  p.sub{color:#a3b3c2;margin:0 0 32px;}
  .grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(260px,1fr));gap:14px;}
  .card{display:block;background:#15293b;border:1px solid rgba(244, 244, 245,.08);border-radius:14px;padding:18px 20px;text-decoration:none;color:inherit;transition:transform .15s,border-color .15s;}
  .card:hover{transform:translateY(-2px);border-color:#fd5812;}
  .card .t{font-family:'Bricolage Grotesque',sans-serif;font-weight:600;font-size:1.05rem;color:#f4f4f5;}
  .card .m{font-family:monospace;font-size:.74rem;color:#a3b3c2;margin-top:6px;}
</style></head><body><div class="wrap">
  <h1>Shared Artifacts</h1>
  <p class="sub">Built with Agent OS · ${items.length} published</p>
  <div class="grid">
${cards || '<p style="color:#6b7d8f">Nothing published yet.</p>'}
  </div>
</div></body></html>`;
}
