...
async function openaiChat(baseUrl: string, model: string, sys: string, user: string, key: string, signal?: AbortSignal, opts?: { noReasoning?: boolean }): Promise<string> {
  ...
  const r = await fetch(url, {
    ...
    // stream:false is explicit because this function parses a single JSON body.
    // OpenRouter defaults to non-streaming, but OmniRoute defaults to SSE — without
    // this it returns "data: {…}" frames and r.json() throws a SyntaxError.
    body: JSON.stringify({ model, max_tokens: 1200, temperature: 0.75, stream: false, ...(opts?.noReasoning ? { reasoning: { enabled: false } } : {}), messages: [{ role: "system", content: sys }, { role: "user", content: user }] }),
  });
